...
protected:
	...
	char * ftheta_equat_postfix;			//Parameter function of the variable theta in the postfix notation
	char * theta0_equat_postfix;			//Initial value of parameter theta, theta = theta0 in the postfix notation


public:
	ProjectionPseudoCylindrical() : Projection<T>(), lat0(45), cart_pole(MAX_LAT, 0.0), lon_dir(NormalDirection2), ftheta_equat(NULL), theta0_equat(NULL), ftheta_equat_postfix(NULL), theta0_equat_postfix(NULL) {}
	ProjectionPseudoCylindrical(const T R_, const T lat0_, const T latp_, const T lonp_, const TTransformedLongtitudeDirection lon_dir_, const char * ftheta_equat_, const char * theta0_equat_, const char * ftheta_equat_postfix_, const char * theta0_equat_postfix_,
		const T lon0_, const T dx_, const T dy_, const T c_, const char * x_equat_, const char * y_equat_, const char * x_equat_postfix_, const char * y_equat_postfix_,
...
public:
	...
	virtual const char * getFThetaEquatPostfix() const { return ftheta_equat_postfix; }
	virtual const char * getTheta0EquatPostfix() const { return theta0_equat_postfix; }
	virtual char * getFThetaEquatPostfix() { return ftheta_equat_postfix; }
	virtual char * getTheta0EquatPostfix() { return theta0_equat_postfix; }
