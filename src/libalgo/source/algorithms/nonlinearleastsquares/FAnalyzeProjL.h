// Description: Functor, compute matrix L for cartometric analysis
...
#ifndef FAnalyzeProjL_H
#define FAnalyzeProjL_H
...
class FAnalyzeProjL
{
        ...
        public:
                ...
                FAnalyzeProjL ( Container <Node3DCartesian <T> *> &nl_test_, Container <Point3DGeographic <T> *> &pl_reference_, typename TMeridiansList <T> ::Type &meridians_, typename TParallelsList <T> ::Type &parallels_,
                ...
                void operator () ( Matrix <T> &X, Matrix <T> &L, Matrix <T> &W, const bool compute_analysis = true )
                {

                        //Compute parameters of the L matrix
                        ...
                        //Set remaining parameters: earth radius and additive constants dx, dy
                        proj->setR ( X ( 0, 0 ) );
                        proj->setDx ( X ( 5, 0 ) );
                        proj->setDy ( X ( 6, 0 ) );

                        //Compute coordinate differences: items of L matrix
                        for ( unsigned int i = 0; i < m; i++ )
                        {
                                ...
                                try
                                {
                                        ...
                                        const T lon_trans = CartTransformation::lonToLonTrans ( pl_reference [i]->getLat(), lon_red, lat_trans, X ( 1, 0 ),  X ( 2, 0 ), NormalDirection ) ;

                                        //Compute new coordinates
                                        x_new = ArithmeticParser::parseEq ( proj->getXEquat(), lat_trans, lon_trans, X ( 0, 0 ), proj->getA(), proj->getB(), proj->getC(), X ( 3, 0 ), proj->getLat1(), proj->getLat2(), false ) + X ( 5, 0 );
                                        y_new = ArithmeticParser::parseEq ( proj->getYEquat(), lat_trans, lon_trans, X ( 0, 0 ), proj->getA(), proj->getB(), proj->getC(), X ( 3, 0 ), proj->getLat1(), proj->getLat2(), false ) + X ( 6, 0 );
                                }
                                ...
                                //Compute coordinate differences: estimated - input
                                L ( i, 0 ) = x_new - nl_test [i]->getX();
                                L ( i + m, 0 ) = y_new - nl_test [i]->getY();
                        }
                }
};
