...
T CharToNum::atof2(const char *num)
{
	...
	{ 
		...
		throw ("ErrorBadData: the strinh has no dezimal place.", "Conversion to number aborted.");
	...
	if (*num == 'e' || *num == 'E'){
		...
		{ 
			...
			throw ("ErrorBadData: the string has no exponent.", "Conversion to number aborted.");
		} 
	...
	{ 
		...
		throw ("ErrorBadData: the next character is not \n.", "Conversion to number aborted.");
	} 
