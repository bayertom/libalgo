...
//Projected point storing cartographic parameters ( cartographic ortions )
template <typename T>
class Point3DCartesianProjected : virtual public Point3DCartesian <T>
{
        ...
        public:
                ...
                Point3DCartesianProjected ( const char * point_label, const T x_, const T y_, const T z_, const T h_, const T k_, const T s_, const T theta_, const TTissotIndicatrix <T> &tiss_, const T w_ ) : Point3DCartesian <T> ( point_label, x_, y_, z_ ),
                        h ( h_ ), k ( k_ ), s ( s_ ), theta ( theta_ ), tiss ( tiss_ ), w ( w_ ) {}
