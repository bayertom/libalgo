...
#include "libalgo/src/structures/list/IndexLists.h"
...
#include "libalgo/src/exceptions/ErrorBadOutput.h"
...
KDNode <Point> *KDTree2D <Point>:: findKDNode ( const Point2 * point, KDNode <Point> **parent ) const
{
        ...
        *parent = NULL;
        ...
        if ( root == NULL )
        {
                throw ErrorBadOutput ( "ErrorBadOutput: can not find KD-Node, ", "no KD-Tree has been constructed." );
        }
...
void KDTree2D <Point>::findAllKNN ( const Point2 * point, Container <Point *, NonDestructable > &knn, unsigned int k ) const
{
        ...
        if ( root == NULL )
        {
                throw ErrorBadOutput ( "ErrorBadOutput: can not find all k-nearest neighbour nodes, ", "no KD-Tree has been constructed." );
        }
...
void KDTree2D <Point>:: printKDTree2D ( const TPrintKDTreeMethod & method, std::ostream * output ) const
{
        ...
        if ( root == NULL )
        {
                throw ErrorBadOutput ( "ErrorBadOutput: can not print KD-tree, ", "no KD-Tree has been constructed." );
        }
