...
#include "libalgo/src/exceptions/ErrorBadData.h"
#include "libalgo/src/exceptions/ErrorIndexOutOfBound.h"
#include "libalgo/src/exceptions/ErrorMathZeroDevision.h"
#include "libalgo/src/exceptions/ErrorMathMatrixDifferentSize.h"

//Forward declaration
template <typename T>
class ErrorMathMatrixDifferentSize;
...
Matrix <T> & Matrix <T> ::operator = ( const Matrix <T> &M )
{
        if ( this != &M )
        {
                ...
                {
                        throw ErrorMathMatrixDifferentSize <Matrix <T> > ( "ErrorMathMatrixDifferentSize: ", " different rows_count count in operator =. Can not assign matrices  ", *this, M );
                }

                //Matrix dimemension are invalid, throw exception
                if ( columns_count != M.columns_count )
                {
                        throw ErrorMathMatrixDifferentSize <Matrix <T> > ( "ErrorMathMatrixDifferentSize: ", " different columns_count count in operator =. Can not assign matrices.  ", *this, M );
                }
...
Matrix<T> Matrix <T> :: operator * ( const Matrix < U > &M ) const
{
        ...
        if ( columns_count !=  m2 )
        {
                throw ErrorMathMatrixDifferentSize <Matrix <U> > ( "ErrorMathMatrixDifferentSize: ", " different rows_count count. Cannot compute A *= B. ", *this, M );
        }
...
Matrix <T> & Matrix <T> :: operator += ( const Matrix <U> &M )
{
        ...
        {
                throw ErrorMathMatrixDifferentSize <Matrix <U> > ( "ErrorMathRange: ", " different columns_count count.  Cannot compute A += B. ", *this, M );
        }
...
Matrix <T> & Matrix <T> :: operator -= ( const Matrix <U> &M )
{
        ...
        {
                throw ErrorMathMatrixDifferentSize <Matrix <U> > ( "ErrorMathMatrixDifferentSize: ", " different columns_count count.  Cannot compute A -= B. ", *this, M );
        }
...
Matrix <T> & Matrix <T> ::operator /= ( const U & val )
{
        ...
        {
                throw ErrorMathZeroDevision <T> ( "ErrorMathZeroDevision: divider is zero. ",  "Can not divide matrix by scalar.", val );
        }
...
void Matrix <T> ::col ( const Matrix <T> &M, const unsigned int c )
{
        ...
        {
                throw ErrorMathMatrixDifferentSize <Matrix <T> > ( "ErrorMathMatrixDifferentSize: ", " invalid dimension of the matrices (rows_count count).  ", *this, M );
        }
...
//Resize matrix
template <typename T>
void Matrix <T> ::res ( const int r, const int c, const T val )
{
        //Not enough rows
        if ( r < 1 )
        {
                throw ErrorBadData ( "ErrorBadData: bad rows count", "matrix can not be resized " );
        }

        //Not enough cols
        if ( c < 1 )
        {
                throw ErrorBadData ( "ErrorBadData: bad columns count", "matrix can not be resized " );
        }
        ...
        *output << '\n';
        ...
        for ( unsigned int i = 0; i < rows_count; i++ )
        {
                *output << "| ";
                ...
                for ( unsigned int j = 0; j < columns_count; j++ )
                {
                        *output <<  std::setw ( 16 ) << std::setprecision ( 7 );
