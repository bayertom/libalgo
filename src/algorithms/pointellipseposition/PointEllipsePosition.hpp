...
unsigned short PointEllipsePosition::getPointEllipsePosition ( const T x, const T y, const T xc, const T yc, const T a, const T b, const T angle )
{
        ...
        if ( a == 0 )
        {
                throw ErrorMathZeroDevision <T> ( "ErrorMathZeroDevision, bad ellipse parameter.", " Can not compute point and ellipse position", a );
        }

        if ( b == 0 )
        {
                throw ErrorMathZeroDevision <T> ( "ErrorMathZeroDevision, bad ellipse parameter.", " Can not compute point and ellipse position", b );
        }
