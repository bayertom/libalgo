...
#include "libalgo/src/structures/tree/KDTree.h"

#include "libalgo/src/algorithms/eucldistance/EuclDistance.h"
#include "libalgo/src/algorithms/pointfaceposition/PointFacePosition.h"
#include "libalgo/src/algorithms/linelineposition/LineLinePosition.h"
#include "libalgo/src/algorithms/faceperimeter/FacePerimeter.h"
...
void GraphAlgorithms::floydWarshall ( const GraphM <T> &g, Matrix <T> &D, Matrix <unsigned int> &P )
{
        ...
        if ( m1 != n1 || m2 != n2 )
        {
                throw ErrorMathMatrixNotSquare <Matrix <T> > ( "ErrorMathMatrixNotSquare: ", " invalid dimension of the matrix (rectangle matrix), can not perform Floyd-Warshall algorithm; (rows_row_index, columns_row_index):  ", D );
        }

        //Different matrices size
        if ( m1 != m2 )
        {
                throw ErrorMathMatrixDifferentSize <Matrix <T> > ( "ErrorMathMatrixDifferentSize: ", " invalid dimension of the matrices, can not perform Floyd-Warshall algorithm; (rows_row_index columns_row_index):  ", D, P );
        }
