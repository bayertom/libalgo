...
T LineLineAngle::getLineLineAngle ( const T x1, const T y1, const T x2, const T y2, const T x3, const T y3, const T x4, const T y4 )
{
        ...
        {
                throw ErrorMathZeroDevision <T> ( "EDivisonByzerroError ", "can not compute line x line angle:  Identical points, denominator = 0.", denominator );
        }
        ...
        {
                throw ErrorMathInvalidArgument <T> ( "EInvalidArgumentError, ", "can not compute line x line angle: fabs(acos(argument) > 1 + ARGUMENT_ROUND_ERROR.", acos_angle );
        }
