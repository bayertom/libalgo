...
#include "libalgo/src/algorithms/vectorvectororientation/VectorVectorOrientation.h"
                ...
                T & n_vect_bis_x, T & n_vect_bis_y )
{
        ...
        if ( norm < MIN_FLOAT )
        {
                throw  ErrorMathZeroDevision <T> ( "ErrorMathZeroDevision, ", "can not compute normalized bisector: 1/norm, norm = 0." );
        }
                ...
                T & n_vect_bis_x, T & n_vect_bis_y )
{
        ...
        if ( nu < MIN_FLOAT )
        {
                throw  ErrorMathZeroDevision <T> ( "ErrorMathZeroDevision", "can not compute normalized bisector: 1/norm (u), norm(u) = 0.", nu );
        }

        if ( nv < MIN_FLOAT )
        {
                throw  ErrorMathZeroDevision <T> ( "ErrorMathZeroDevision, ", "can not compute normalized bisector: 1/norm (v), norm(v) = 0.", nv );
        }
