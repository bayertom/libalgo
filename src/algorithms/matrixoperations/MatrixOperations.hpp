...
#include "libalgo/src/comparators/indexComp.h"

#include "libalgo/src/io/File.h"

#include "libalgo/src/exceptions/ErrorMathOverflow.h"
#include "libalgo/src/exceptions/ErrorMathMatrixDifferentSize.h"
#include "libalgo/src/exceptions/ErrorMathMatrixSingular.h"
#include "libalgo/src/exceptions/ErrorMathMatrixNotSquare.h"
#include "libalgo/src/exceptions/ErrorMathMatrixNotPositiveDefinite.h"
...
namespace MatrixOperations
{
	...
	T sumCol(const Matrix <T> &A, const unsigned int col)
	{
		...
		if (col < 0 || col > n)
		{
			throw ErrorIndexOutOfBound("ErrorIndexOutOfBound: ", "can not compute col sum, invalid col (col < 0 || col > columns_count)");
		}
	...
	T median(const Matrix <T>  &A, const unsigned int col)
	{
		...
		if (col < 0 || col > n)
		{
			throw ErrorIndexOutOfBound("ErrorIndexOutOfBound: ", "can not compute median, invalid col (col < 0 || col > columns_count)");
		}
	...
	Matrix <T> diag(const Matrix <T> &A, const int offset)
	{
		...
		if ((n > 1) || (m == 1) && (n == 1))
		{
			...
			if (offset > n - 1)
			{
				throw ErrorBadData("ErrorBadData: offset value to large, offset > n. ", "Can not get diagonal of the matrix.");
			}

			//Bad offset: to low
			if (offset + m < 0)
			{
				throw ErrorBadData("ErrorBadData: offset value to small, offset + n < 0. ", "Can not get diagonal of the matrix.");
			}
	...
	Matrix <T> tridiag(const Matrix <T> &M, Matrix <T> &l, Matrix <T> &r)
	{
		...
		if (m != n)
		{
			throw ErrorMathMatrixNotSquare <Matrix <T> >("ErrorMathMatrixNotSquare: ", " invalid dimension of the matrix (rectangle matrix), can not compute tridiagonal matrix; (rows_count, columns_count):  ", M);
		}
		...
		if (m != m2)
		{
			throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " different amount of rows of M, L, can not compute tridiagonal matrix:  ", M, l);
		}

		//Different amount of rows
		if (m != m3)
		{
			throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " different amount of rows of M, R, can not compute tridiagonal matrix:  ", M, r);
		}
		...
		R(i, 0) = norm(r);
		E(i, 0) = norm(l);
		V.submat(r * (1.0 / R(i, 0)), 0, i);
		W.submat(l * (1.0 / E(i, 0)), 0, i);
	...
	Matrix <T> tridiagh(Matrix <T> A)
	{
		...
		if (m != n)
		{
			throw ErrorMathMatrixNotSquare <Matrix <T> >("ErrorMathMatrixNotSquare: ", " invalid dimension of the matrix (rectangle matrix), can not compute tridiagonal matrix; (rows_count, columns_count):  ", A);
		}

		//Symmetric matrix
		if (!isequal(A, trans(A)))
		{
			throw ErrorBadData("ErrorBadData: Matrix A is not symmetric. ", "Can not compute tridiagonal matrix.");
		}
	...
	void sort(Matrix <T> &A, Matrix <unsigned int> &IX, const int c)
	{
		...
		if (m1 != m2)
		{
			throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " different rows count for A, IX, can not sort a matrix:  ", A, IX);
		}

		//Test number of columns A and IX
		if (n1 != n2)
		{
			throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " different columns count for A, IX, can not sort a matrix:  ", A, IX);
		}

		//Test, if c is correct
		if ((c > n1 - 1) || (c < -1))
		{
			throw ErrorIndexOutOfBound("ErrorIndexOutOfBound: ", "can not sort matrix by a column c, invalid c (c < -1 || col > columns_count)");
		}
	...
	void sortrows(Matrix <T> &A, Matrix <unsigned int> &IX, const int c)
	{
		...
		if (m1 != m2)
		{
			throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " different rows count for A, IX, can not sort rows of a matrix:  ", A, IX);
		}

		//Test number of columns A and IX
		if (n2 != 1)
		{
			throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " IX does not have 1 column, can not sort rows of a matrix:  ", A, IX);
		}

		//Test, if c is correct
		if ((c > n1 - 1) || (c < -1))
		{
			throw ErrorIndexOutOfBound("ErrorIndexOutOfBound: ", "can not sort rows of the matrix by a column c, invalid c (c < -1 || col > columns_count)");
		}
		...
		for (unsigned int i = 0; i < m1; i++)
		{
			...
			AP.submat(A(ix[i], ix[i], 0, n1 - 1), i, 0);
		}
	...
	Matrix <T> gem(Matrix <T> A, Matrix <T> B)
	{
		...
		if (m1 != m2)
		{
			throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " invalid dimension of the matrices, can not perform GEM; (rows_count != columns_count):  ", A, B);
		}

		//Rectangle matrix
		if (m1 != n1)
		{
			throw ErrorMathMatrixNotSquare <Matrix <T> >("ErrorMathMatrixNotSquare: ", " invalid dimension of the matrix (rectangle matrix), can not perform GEM; (rows_count, columns_count):  ", A, B);
		}
	...
	void gemFSign(Matrix <T> &A_triangle, Matrix <T> *B)
	{
		...
		for (int k = 0; k < n; k++)
		{
			...
			for (unsigned int i = k + 1; i < n; i++)
			{
				...
				{
					throw ErrorMathMatrixSingular <Matrix <T> >("ErrorMathMatrixSingular: ", " singular matrix, cannot perform GEM (forward); (rows_count, columns_count):  ", A_triangle);
				}
			}
	...
	void gemB(Matrix <T> &A, const Matrix <T> &B, Matrix <T> &X)
	{
		...
		for (int i = n - 1; i >= 0; i--)
		{
			...
			{
				throw ErrorMathMatrixSingular <Matrix <T> >("ErrorMathMatrixSingular: ", " singular matrix, cannot perform GEM (backward); (rows_count, columns_count):  ", A);
			}
	...
	T det(Matrix <T> A)
	{
		...
		if (m != n)
		{
			throw ErrorMathMatrixNotSquare <Matrix <T> >("ErrorMathMatrixNotSquare: ", " invalid dimension of the matrix (rectangle matrix), can not compute determinant; (rows_count, columns_count):  ", A);
		}
	...
	void lu(Matrix <T> A, Matrix <T> &L, Matrix <T> &U, Matrix <T> &P)
	{
		...
		if (m != n)
		{
			throw ErrorMathMatrixNotSquare <Matrix <T> >("ErrorMathMatrixNotSquare: ", " invalid dimension of the matrix (rectangle matrix), can not perform LU decomposition; (rows_count, columns_count):  ", A);
		}
		...
		for (unsigned int j = 0; j < n; j++)
		{
			...
			if ((j != i_pivot) && (i_pivot < n))
			{
				...
				U.submat(U(j, j, 0, n - 1), i_pivot, 0);
				U.submat(U_temp, j, 0);
		...
		for (int i = 0; i < n; i++)
		{
			...
			while (orthogonalize)
			{
				for (int j = 0; j <= i - 1; j++)
				{
					...
					Q.submat(Q(0, m - 1, i, i) - Q(0, m - 1, j, j) * S(0, 0), 0, i);
				}
	...
	void qr(const Matrix <T> &A, Matrix <T> &Q, Matrix <T> &R, Matrix <unsigned int> &P)
	{
		...
		if (m < n)
		{
			throw ErrorBadData("ErrorBadData: invalid dimension of the matrix A, m < n.", "Can not compute QR decomposition with columns pivoting.");
		}

		//Bad dimensions of Q
		if ((Q.rows() != m) || (Q.cols() != m))
		{
			throw ErrorBadData("ErrorBadData: invalid dimension of the matrix Q(m, m).", "Can not compute QR decomposition with columns pivoting.");
		}

		//Bad dimensions of R
		if ((R.rows() != m) || (R.cols() != n))
		{
			throw ErrorBadData("ErrorBadData: invalid dimension of the matrix R (m, n).", "Can not compute QR decomposition with columns pivoting.");
		}

		//Bad dimensions of P
		if ((P.rows() != n) || (P.cols() != n))
		{
			throw ErrorBadData("ErrorBadData: Matrix A is rank deficient (m <n).", "Can not compute QR decomposition with columns pivoting, must be of m >=n.");
		}

		//First iteration: R = A, Q = E, P = E
		R = A;

		for (unsigned int i = 0; i < m; i++) Q(i, i) = 1.0;

		for (unsigned int i = 0; i < n; i++) P(i, i) = 1.0;
		...
		for (unsigned int i = 0; i < n - 1; i++)
		{
			...
			if (permut_index != i)
			{
				...
				P.submat(P(0, n - 1, permut_index, permut_index), 0, i);
				P.submat(TMP, 0, permut_index);

				//Swap R matrix
				const Matrix <T> TMP2 = R(0, m - 1, i, i);
				R.submat(R(0, m - 1, permut_index, permut_index), 0, i);
				R.submat(TMP2, 0, permut_index);
			...
			col_norms.submat(col_norms(0, 0, i + 1, n - 1) - col_norm, 0, i + 1);
		}
	}
	...
	bool posdef(const Matrix <T> &A)
	{
		...
		if (m != n)
		{
			throw ErrorMathMatrixNotSquare <Matrix <T> >("ErrorMathMatrixNotSquare: ", " invalid dimension of the matrix (rectangle matrix), can not compute inverse matrix; (rows_count, columns_count):  ", A);
		}
	...
	Matrix <T> chol(Matrix <T> A, const TCholeskyFactorization f)
	{
		...
		{
			throw ErrorMathMatrixNotPositiveDefinite <Matrix <T> >("ErrorMathMatrixNotPositiveDefinite: ", " invalid dimension of the matrix (rectangle matrix), can not compute Cholesky decomposition:  ", A);
		}
	...
	void gill(const Matrix <T> &A, Matrix <T> &R, Matrix <T> &E, bool &indefinite, const T max_error)
	{
		...
		for (unsigned int i = 0; i < m; i++)
		{
			L.submat(L(0, m - 1, i, i) * sqrt(D(i, 0)), 0, i);
		}
	...
	void gill(const Matrix <T> &A, Matrix <T> &L, Matrix <T> &d, Matrix <T> &e, Matrix <T> &v, const T max_error)
	{
		...
		for (unsigned int i = 0; i < m; i++)
		{
			...
			if (i > 0)
			{
				if (i < n - 1)
				{
					C.submat(A(i + 1, n - 1, i, i) - C(i + 1, n - 1, 0, i - 1)  * trans(L(i, i, 0, i - 1)), i + 1, i);
				}
			}

			else
			{
				C.submat(A(i + 1, n - 1, i, i), i + 1, i);
			}
	...
	Matrix <T> inv(Matrix <T> A)
	{
		...
		if (max_val > MAX_DOUBLE)
			throw ErrorMathOverflow <T>("ErrorMathOverflow: bad scaled matrix, can not compute inverse matrix. ", "Max item > MAX_FLOAT.", max_val);
	...
	void eig(const Matrix <T> &A, Matrix<T> &V, Matrix<T> &L, const unsigned int max_iter, const T tolerance)
	{
		...
		if (m != n)
		{
			throw ErrorMathMatrixNotSquare <Matrix <T> >("ErrorMathMatrixNotSquare: ", " invalid dimension of the matrix (rectangle matrix), can not compute eigenvalues; (rows_count, columns_count):  ", A);
		}
		...
		if (!positive_definite)
		{
		throw ErrorMathMatrixNotPositiveDefinite <Matrix <T> >("ErrorMathMatrixNotPositiveDefinite: ", " invalid dimension of the matrix (rectangle matrix), can not compute eigenvalue decomposition:  ", A);
		}
		*/
		...
		for (unsigned int iter = 0; (iter < max_iter) && (index != 0); iter++)
		{
			...
			for (unsigned int i = 1; i <= index; i++)
			{
				...
				V.submat(V(0, mb - 1, i - 1, i - 1) * (-S(i, 0)) + V(0, mb - 1, i, i) * D(i, 0), 0, i);
				V.submat(CO, 0, i - 1);
			}
		...
		{
			//Copy row to permutated matrix
			VP.submat(V(IX(i, 0), IX(i, 0), 0, m - 1), i, 0);
		}
	...
	void svd(const Matrix <T> &A, Matrix <T> &U, Matrix <T> &B, Matrix <T> &V, unsigned int max_iterations)
	{
		...
		{
			throw ErrorBadData("ErrorBadData: invalid dimension of the matrix A.", "Can not compute SVD.");
		}

		//Bad dimensions of B
		if ((B.rows() != m) || (B.cols() != n))
		{
			throw ErrorBadData("ErrorBadData: invalid dimension of the matrix B.", "Can not compute SVD.");
		}

		//Bad dimensions of V
		if ((V.rows() != n) || (V.cols() != n))
		{
			throw ErrorBadData("ErrorBadData: invalid dimension of the matrix V.", "Can not compute SVD.");
		}
	...
	void bidiag(const Matrix <T> &A, Matrix <T> &U, Matrix <T> &B, Matrix <T> &V)
	{
		...
		if ((U.rows() != m) || (U.cols() != m))
		{
			throw ErrorBadData("ErrorBadData: invalid dimension of the matrix A.", "Can not compute bidiagonalization.");
		}

		//Bad dimensions of B
		if ((B.rows() != m) || (B.cols() != n))
		{
			throw ErrorBadData("ErrorBadData: invalid dimension of the matrix B.", "Can not compute bidiagonalization.");
		}

		//Bad dimensions of V
		if ((V.rows() != n) || (V.cols() != n))
		{
			throw ErrorBadData("ErrorBadData: invalid dimension of the matrix V.", "Can not compute bidiagonalization.");
		}
	...
	Matrix <T> hous(const Matrix <T> &A, const unsigned int i, const unsigned int j)
	{
		...
		if (n > 1)
		{
			throw ErrorBadData("ErrorBadData: Matrix A is not a column vector. ", "Can not compute the Householder vector.");
		}

		//Bad index condition: i > j
		if (i > j)
		{
			throw ErrorBadData("ErrorBadData: index i > j. ", "Can bot compute the Householder vector.");
		...
		if (j > m - 1)
		{
			throw ErrorBadData("ErrorBadData: index i > n - 1 . ", "Can bot compute the Householder vector.");
		}

		//Initialize Householder vector
		Matrix <T> H(m, n);
		H.submat(A(i, j, 0, 0), i, 0);
	...
	Matrix <T> hous(const Matrix <T> &A, const unsigned int i)
	{
		...
		if (i > m - 1)
		{
			throw ErrorBadData("ErrorBadData: index i > n - 1 . ", "Can not compute the Householder vector.");
		}
		...
		V.submat(D(1, m - i - 1, 0, 0) / (2 * p), 1, 0);
	...
	Matrix <T> load(const char *file, const bool print_exception, std::ostream * output)
	{
		try
		{
			...
			//Throw exception
			else throw ErrorBadData("ErrorBadData: no items in matrix. ", "Can not load matrix from a file.");
		}
