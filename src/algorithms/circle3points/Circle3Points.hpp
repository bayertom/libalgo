...
void Circle3Points::getCentreAndDiameterCircle ( const Point3DCartesian <T> * p1, const Point3DCartesian <T> * p2, const Point3DCartesian <T> * p3, T & xc, T & yc, T & r )
{
        ...
        {
                throw ErrorMathZeroDevision <T> ( "ErrorMathZeroDevision: can not compute mid point of the circle. ", "Circle given by 3 points, 2 points are identical.", denom1 );
        }
        ...
        {
                throw ErrorMathZeroDevision <T> ( "ErrorMathZeroDevision: can not compute mid point of the circle.", "Circle given by 3 points, 2 points are identical.", denom2 );
        }
        ...
        {
                throw ErrorBadData ( "ErrorBadData: too big coordinates of the circle mid point, xc > MAX_POINT_COORDINATE", " (MAX_POINT_COORDINATE = 1e+10)." ) ;
        }

        //Y coordinates are too big, throw exception
        if ( fabs ( yc ) > MAX_POINT_COORDINATE )
        {
                throw ErrorBadData ( "ErrorBadData: too big coordinates of the circle mid point, yc > MAX_POINT_COORDINATE", " (MAX_POINT_COORDINATE = 1e+10)." ) ;
        }
