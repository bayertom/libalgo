...
#include "libalgo/src/exceptions/ErrorParse.h"
...
void NumDerivative::computeFunctionValues(Function function, const Matrix <T> &args, const TDerivativeVariable deriv_var, const T deriv_step, T * values, T * fvalues, const bool print_exceptions)
{
        ...
        if ( deriv_var > args.cols() -  1 )
                throw ErrorBadData ( "ErrorBadData: not enough arguments (deriv_index > arg.size() - 1).", "Can not compute the partial derivative..." );
