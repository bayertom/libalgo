...
#include "libalgo/src/structures/line/HalfEdge.h"

#include "libalgo/src/algorithms/lawsonorientedwalk/LawsonOrientedWalk.h"
#include "libalgo/src/algorithms/convexquadrilateral/ConvexQuadrilateral.h"
#include "libalgo/src/algorithms/swappingcriteria/SwappingCriteria.h"

#include "libalgo/src/comparators/removeSimplexHalfEdge.h"
#include "libalgo/src/comparators/sortPointsByX.h"
#include "libalgo/src/comparators/sortPointsByY.h"
#include "libalgo/src/comparators/sortPointsToBins.h"
#include "libalgo/src/comparators/isEqualPointByPlanarCoordinates.h"
...
void DT2D::createSimplexTriangle ( Container <Node3DCartesian <T> *> &pl, Container <HalfEdge <T> *> &half_edges_dt )
{
        ...
        if ( dx == 0 || dy == 0 )
                throw ErrorBadData ( "ErrorBadData: All input points are colinear. ", "Can not contruct Delaunay triangulation. " );
...
void DT2D::DTInsertPoint ( Node3DCartesian <T> *p, const Node3DCartesian <T> *s1, const Node3DCartesian <T> *s2, const Node3DCartesian <T> *s3, HalfEdge <T> **e1, Container <HalfEdge <T> *>  &half_edges_dt )
{
        ...
        //Throw exception
        catch ( ErrorMathZeroDevision <T> & error )
        {
                //Set NULL pointers
                ...
                //Throw exception
                throw ErrorMathZeroDevision <T> ( "ErrorMathZeroDevision: ", "Delaunay triangulation: Can not create new triangles for inserted point p.", error.getArg() );
        }
}
