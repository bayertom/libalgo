...
void Graticule::computeMeridian ( const Projection <T> *proj, const TMinMax<T> &lat_interval, const T d_lat, const T alpha, const TGraticuleType grat_type, Meridian <T> &meridian, Container <Node3DCartesian <T> *> *points, unsigned int & index )
{
        ...
        for ( unsigned int i = 0; ( lat  < lat_interval.max_val + d_lat ) && ( lat_interval.min_val < lat_interval.max_val ); lat += d_lat, i++ )
        {
			...
			//Get rotation
			/*
			const T alpha = proj->getAlpha();
			const T x_mass_test = proj->getXMassTest();
			const T y_mass_test = proj->getYMassTest();
			const T x_mass_reference = proj->getXMassReference();
			const T y_mass_reference = proj->getYMassReference();
			...
			*/
			...
			{
				// x = ( x_temp -dx - x_mass_reference ) * cos ( alpha * M_PI / 180 ) - ( y_temp - dy - y_mass_reference ) * sin ( alpha * M_PI / 180 ) + x_mass_test ;
                                // y = ( x_temp -dx - x_mass_reference ) * sin ( alpha * M_PI / 180 ) + ( y_temp - dy - y_mass_reference ) * cos ( alpha * M_PI / 180 ) + y_mass_test;
