...
void Outliers::findOutliersME(const Matrix <T> &P, const Matrix <T> &Q, const T k, const T tol, const TMEstimatorsScheme me_scheme, TMEstimatorsWeightFunction me_weight_function, const unsigned int max_iter, Matrix <T> &W, Matrix <unsigned int> &I, Matrix <T> &Eps,  T &f_init, T &f, unsigned int &iter)
{
	...
	if (m1 != m2)
	{
		throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " different rows count for P, Q, can not find outliers:  ", P, Q);
	}

	//Test number of columns A and IX
	if (n1 != n2)
	{
		throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " different columns count for P, Q, can not find outliers:  ", P, Q);
	}

	//W matrix must be square
	if (m3 != n3)
	{
		throw ErrorMathMatrixNotSquare <Matrix <T> >("ErrorMathMatrixMatrixNotSquare: ", " W matrix must be square:  ", W);
	}

	//Are there enough points?
	if ((me_scheme == ScaleScheme) && (m1 < 2) || (me_scheme == !ScaleScheme) && (m1 < 4))
	{
		throw ErrorBadData("ErrorBadData: not enough points, ", " can not find outliers using M-estimators.");
	}

	//W and P
	if (m3 != 2 * m1)
	{
		throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " incorrect size of P(n, n), and W(2*n,2*n), :  ", P, W);
	...
	if (m3 != m4)
	{
		throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " different rows count for W, I, can not find outliers:  ", W, I);
	}

	//I and Eps
	if (m4 != m5)
	{
		throw ErrorMathMatrixDifferentSize <Matrix <T> >("ErrorMathMatrixDifferentSize: ", " different rows count for I, Eps, can not find outliers:  ", I, Eps);
	}
	...
	{
		A.submat(Q(0, m1 - 1, 0, 0), 0, 0);
		A.submat(Q(0, m1 - 1, 1, 1), m1, 0);
	}
	...
	{
		A.submat(-1.0 * Q(0, m1 - 1, 1, 1), 0, 1);
		A.submat(Q(0, m1 - 1, 0, 0), m1, 1);
	...
	//Create Y vector
	Matrix <T> Y(2 * m1, 1);
	Y.submat(P(0, m1 - 1, 0, 0), 0, 0);
	Y.submat(P(0, m1 - 1, 1, 1), m1, 0);
