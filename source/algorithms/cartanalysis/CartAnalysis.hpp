	...
	unsigned int & total_created_or_thrown_samples, std::ostream * output)
{
	...
	//Create jitterple for analyzed projection from command line and set flag for this jitterple
	if (analysis_parameters.analyzed_projections.size() > 0)
	{
		...
		for (typename TItemsList<Projection <T> *> ::Type::iterator i_projections = analysis_parameters.analyzed_projections.begin(); i_projections != analysis_parameters.analyzed_projections.end(); ++i_projections)
		{
			...
			//Remove singular points to prevent throwing a jitterple
			...
			if (nl_test.size() != nl_test_non_sing.size())
			{
				...
				//Set flag to true, used for a jitterple using non-singular sets
			...
			try
			{
				Sample <T> analyzed_jitterple;
				(void)computeAnalysisForOneSample(*p_nl_test, *p_pl_reference, meridians, parallels, faces_test, analyzed_proj, analysis_parameters, analyzed_jitterple, singular_points_found, total_created_and_analyzed_samples_projection, output);

				//Add result to the list
				if (total_created_and_analyzed_samples_projection) sl.push_back(analyzed_jitterple);
			}
	...
	for (typename TItemsList <Projection <T> *> ::Type::const_iterator i_projections = pl.begin(); i_projections != pl.end(); ++i_projections)
	{
		...
		*output << (*i_projections)->getProjectionName() << ": ";
		...
		for (unsigned int i = 0; i < proj_pole_positions_list.size(); i++)
		{
			...
			//Try to remove singular points to prevent a throwing of jitterple only if  cartographic pole coordinates latp, lonp change
			...
			try
			{
				...
				computeAnalysisForOneSample(*p_nl_test_non_sing, *p_pl_reference_non_sing, meridians, parallels, faces_test, *i_projections, analysis_parameters, analyzed_jitterple, singular_points_found, created_samples, output);
			...
			//Print "." for every 500-th jitterple to the log
		...
		*output << (*i_projections)->getProjectionName() << ": ";
		...
		try
		{
			...
			if ((analysis_parameters.analyze_normal_aspect) && ((*i_projections)->getCartPole().getLat() == MAX_LAT))
			{
				...
				CartAnalysis::computeAnalysisForOneSample(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections, analysis_parameters_test, jitterple_test, false, total_samples_test, output);

				//Get initialize Earth radius using the similarity transformation
				const T R_init = (*i_projections)->getR() / jitterple_test.getScaleHelT();
				...
				for (unsigned int j = 0; j < 1; j++)
				{
					...
					if (analysis_parameters.analysis_method == SimplexMethod)
						min_cost = SimplexMethod::NelderMead(FAnalyzeProjV2S <T>(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections, analysis_parameters, ObliqueAspect, best_jitterple,
						total_created_and_analyzed_samples_projection, it_res, output), XMIN, XMAX, W, X, Y, V, iterations, eps, max_iter, output);
					else if (analysis_parameters.analysis_method == SimplexRotMethod)
						min_cost = SimplexMethod::NelderMead(FAnalyzeProjV3S <T>(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections, analysis_parameters, ObliqueAspect, best_jitterple,
						total_created_and_analyzed_samples_projection, it_res, XMIN, XMAX, output), XMIN, XMAX, W, X, Y, V, iterations, eps, max_iter, output);
					else if (analysis_parameters.analysis_method == SimplexRot2Method)
						min_cost = SimplexMethod::NelderMead(FAnalyzeProjV4S <T>(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections, R_def, q1, q2, analysis_parameters, ObliqueAspect, best_jitterple,
						total_created_and_analyzed_samples_projection, it_res, output), XMIN, XMAX, W, X, Y, V, iterations, eps, max_iter, output);
					else if (analysis_parameters.analysis_method == SimplexShiftsMethod)
						min_cost = SimplexMethod::NelderMead(FAnalyzeProjVS <T>(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections, analysis_parameters, ObliqueAspect, best_jitterple,
						total_created_and_analyzed_samples_projection, it_res, output), XMIN, XMAX, W, X, Y, V, iterations, eps, max_iter, output);
					...
					best_jitterple.setHomotheticTransformationPercMatch((unsigned int)match_ratio);
					//best_jitterple.setHomotheticTransformationMatchedPointsIndices ( matched_points );

					//Set residuals
					analysis_parameters.analysis_method == NonLinearLeastSquaresMethod ? best_jitterple.setHomotheticTransformationRatio(sqrt(min_cost / (m - 1))) : best_jitterple.setHelmertTransformationRatio(sqrt(min_cost / (m - 1)));
					...
					if (total_created_and_analyzed_samples_projection)
					{
						...
						{
							//Set properties of the jitterple
							best_jitterple.setR(X(0, 0));
							best_jitterple.setLatP(MAX_LAT);
							best_jitterple.setLonP(0.0);
							best_jitterple.setLat0(X(0, 3));
							best_jitterple.setLon0(X(0, 4));
							best_jitterple.setC(X(0, 5));

							if (analysis_parameters.analysis_method == SimplexRotMethod)
								best_jitterple.setAlpha(X(0, 6));

							//Add jitterple to the list
							sl.push_back(best_jitterple);
						}
					...
					//Test result
					if (best_jitterple.getHomotheticTransformationRatio() < analysis_parameters.max_error)
					{
						effic++;
						cost_good += min_cost;
					}
				...
				((*i_projections)->getLatPInterval().min_val != (*i_projections)->getLatPInterval().max_val))
			{
				...
				if (total_created_and_analyzed_samples_projection_before <  total_created_and_analyzed_samples_projection)
				{
						...
						((X(0, 3) >= lat0_interval.min_val) && (X(0, 3) <= lat0_interval.max_val)))
					{
						//Set properties of the jitterple
						best_jitterple.setR(X(0, 0));
						best_jitterple.setLatP(0.0);
						best_jitterple.setLonP(X(0, 2));
						best_jitterple.setLat0(X(0, 3));
						best_jitterple.setLon0(X(0, 4));
						best_jitterple.setC(X(0, 5));
				...
				((*i_projections)->getLatPInterval().min_val != (*i_projections)->getLatPInterval().max_val))
			{
				...
				for (unsigned int j = 0; j < 1; j++)
				{
					...
					batchTestSimplex(nl_test, pl_reference, *i_projections, ObliqueAspect, meridians, parallels, faces_test, R, lat_p, lon_p, lat_0, iterations, eps, max_iter, max_diff, best_jitterple, analysis_parameters, total_created_and_analyzed_samples_projection, output);
				...
				for (int k = 0; k < 1; k++)
				{
					...
					for (unsigned int i = 0; i < (lonp_interval_heur.min_val <= lonp_interval_heur.max_val ? 2 : 3); i++)
					{
						...
						min_cost = (analysis_parameters.analysis_method == DifferentialEvolutionMethod ?
							DifferentialEvolution::diffEvolution(FAnalyzeProjV2DE <T>(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections, analysis_parameters, NormalAspect, best_jitterple,
							...
							DifferentialEvolution::diffEvolution(FAnalyzeProjV3DE <T>(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections, analysis_parameters, NormalAspect, best_jitterple,
				...
				for (int k = 0; k < 1; k++)
				{
					...
					for (unsigned int i = 0; i < (lonp_interval_heur.min_val <= lonp_interval_heur.max_val ? 1 : 2); i++)
					{
						...
						*/
						...
						//batchTestDiffEvolutionSchema(nl_test, pl_reference, *i_projections, ObliqueAspect, meridians, parallels, faces_test, R, lat_p, lon_p, lat_0, iterations, eps, max_gen, best_jitterple, analysis_parameters, total_created_and_analyzed_samples_projection, output);
						batchTestDiffEvolutionAdaptiveControl(nl_test, pl_reference, *i_projections, ObliqueAspect, meridians, parallels, faces_test, R, lat_p, lon_p, lat_0, iterations, eps, max_gen, best_jitterple, analysis_parameters, DERandBest1Strategy, total_created_and_analyzed_samples_projection, output);
						//batchTestDiffEvolutionAdaptiveControl(nl_test, pl_reference, *i_projections, ObliqueAspect, meridians, parallels, faces_test, R, lat_p, lon_p, lat_0, iterations, eps, max_gen, best_jitterple, analysis_parameters, DERand1Strategy, total_created_and_analyzed_samples_projection, output);
						//batchTestDiffEvolutionAdaptiveControl(nl_test, pl_reference, *i_projections, ObliqueAspect, meridians, parallels, faces_test, R, lat_p, lon_p, lat_0, iterations, eps, max_gen, best_jitterple, analysis_parameters, DETargetToBest1Strategy, total_created_and_analyzed_samples_projection, output);
		...
		try
		{
			...
			if ((analysis_parameters.analyze_normal_aspect) && ((*i_projections)->getCartPole().getLat() == MAX_LAT))
			{
				...
				if (analysis_parameters.analysis_method == NonLinearLeastSquaresMethod)
				{
					...
					min_cost = NonLinearLeastSquares::BFGSH(FAnalyzeProjJ2 <T>(nl_test, pl_reference, (*i_projections), NormalAspect, analysis_parameters.print_exceptions, j_iter), FAnalyzeProjV2 <T>(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections,
						analysis_parameters, NormalAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X, Y, V, A, B, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
				}
				...
				else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRotMethod)
				{
					...
					T min_cost = NonLinearLeastSquares::BFGSH(FAnalyzeProjJ4 <T>(nl_test, pl_reference, (*i_projections), NormalAspect, R_sphere, q1, q2, analysis_parameters.print_exceptions, j_iter), FAnalyzeProjV4 <T>(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections,
						R_def, q1, q2, analysis_parameters, NormalAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X_init, Y, V, A_init, B_init, iterations, alpha_mult, nu, eps, 0.5 * max_iter, max_diff, output);
					...
					min_cost = NonLinearLeastSquares::BFGSH(FAnalyzeProjJ3 <T>(nl_test, pl_reference, nl_projected, (*i_projections), NormalAspect, x_mass_reference, y_mass_reference, analysis_parameters.print_exceptions, j_iter), FAnalyzeProjV3 <T>(nl_test, pl_reference, nl_projected, meridians, parallels, faces_test, *i_projections,
						x_mass_reference, y_mass_reference, analysis_parameters, NormalAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X, Y, V, A, B, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
				}
				...
				//batchTestNLSP(nl_test, pl_reference, nl_projected, *i_projections, NormalAspect, meridians, parallels, faces_test, W, X, Y, V, iterations, alpha_mult, nu, eps, max_iter, max_diff, x_mass_reference, y_mass_reference, best_jitterple, analysis_parameters, total_created_and_analyzed_samples_projection, output);
				//batchTestG(nl_test, pl_reference, nl_projected, *i_projections, NormalAspect, meridians, parallels, faces_test, iterations, alpha_mult, nu, eps, max_iter, max_diff, x_mass_reference, y_mass_reference, n_par, best_jitterple, analysis_parameters, sl, total_created_and_analyzed_samples_projection, output);
				//return;
				//*********************************
				...
				if (total_created_and_analyzed_samples_projection > 0)
				{
						...
						//                 ( ( X ( 3, 0 ) >= lat0_interval.min_val ) && ( X ( 0, 3 ) <= lat0_interval.max_val ) ) )
					{
						...
						best_jitterple.setLatP(MAX_LAT);
						best_jitterple.setLonP(0.0);
						best_jitterple.setLat0(X(3, 0));
						best_jitterple.setLon0(X(4, 0));
						best_jitterple.setC(X(5, 0));

						if (analysis_parameters.analysis_method == NonLinearLeastSquaresRotMethod)
							best_jitterple.setAlpha(X(6, 0));
						else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRot2Method)
							best_jitterple.setAlpha(alpha);
				...
				((*i_projections)->getLatPInterval().min_val != (*i_projections)->getLatPInterval().max_val))
			{
				...
				if (analysis_parameters.analysis_method == NonLinearLeastSquaresMethod)
				{
					...
					analysis_parameters, TransverseAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X, Y, V, A, B, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
				}
				...
				else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRotMethod)
				{
					...
					min_cost = NonLinearLeastSquares::BFGSH(FAnalyzeProjJ4 <T>(nl_test, pl_reference, (*i_projections), ObliqueAspect, R_sphere, q1, q2, analysis_parameters.print_exceptions, j_iter), FAnalyzeProjV4 <T>(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections,
						R_def, q1, q2, analysis_parameters, ObliqueAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X_init, Y, V, A_init, B_init, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
					...
					min_cost = NonLinearLeastSquares::BFGSH(FAnalyzeProjJ3 <T>(nl_test, pl_reference, nl_projected, (*i_projections), TransverseAspect, x_mass_reference, y_mass_reference, analysis_parameters.print_exceptions, j_iter), FAnalyzeProjV3R <T>(nl_test, pl_reference, nl_projected, meridians, parallels, faces_test, *i_projections,
						x_mass_reference, y_mass_reference, analysis_parameters, TransverseAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X, Y, V, A, B, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
				}
				...
				((*i_projections)->getLatPInterval().min_val != (*i_projections)->getLatPInterval().max_val))
			{
				...
				//Remember old values
				T lon0_init = lon0;
				//T dx_init = jitterple_test.getDx();
				//T dy_init = jitterple_test.getDy();
				...
				*/
				...
				if (analysis_parameters.analysis_method == NonLinearLeastSquaresMethod)
				{
					...
					min_cost = NonLinearLeastSquares::BFGSH(FAnalyzeProjJ2 <T>(nl_test, pl_reference, (*i_projections), ObliqueAspect, analysis_parameters.print_exceptions, j_iter), FAnalyzeProjV2 <T>(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections,
						analysis_parameters, ObliqueAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X, Y, V, A, B, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
				}
				...
				else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRotMethod)
				{
					...
					T R_sphere = 6380;
					min_cost = NonLinearLeastSquares::BFGSH(FAnalyzeProjJ4 <T>(nl_test, pl_reference, (*i_projections), ObliqueAspect, R_sphere, q1, q2, analysis_parameters.print_exceptions, j_iter), FAnalyzeProjV4 <T>(nl_test, pl_reference, meridians, parallels, faces_test, *i_projections,
						R_def, q1, q2, analysis_parameters, ObliqueAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X_init, Y, V, A_init, B_init, iterations, alpha_mult, nu, eps, 0.3 * max_iter, max_diff, output);
					...
					min_cost = NonLinearLeastSquares::BFGSH(FAnalyzeProjJ3 <T>(nl_test, pl_reference, nl_projected, (*i_projections), ObliqueAspect, x_mass_reference, y_mass_reference, analysis_parameters.print_exceptions, j_iter), FAnalyzeProjV3 <T>(nl_test, pl_reference, nl_projected, meridians, parallels, faces_test, *i_projections,
						x_mass_reference, y_mass_reference, analysis_parameters, ObliqueAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X, Y, V, A, B, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
				}
				...
				*/
				...
				//batchTestShifts(nl_test, pl_reference, nl_projected, *i_projections, ObliqueAspect, meridians, parallels, faces_test, iterations, alpha_mult, nu, eps, max_iter, max_diff, x_mass_reference, y_mass_reference, best_jitterple, analysis_parameters, total_created_and_analyzed_samples_projection, output);
				batchTestNLSP(nl_test, pl_reference, nl_projected, *i_projections, ObliqueAspect, meridians, parallels, faces_test,  R, lat_p,  lon_p, lat_0, iterations, alpha_mult, nu, eps, max_iter, max_diff, x_mass_reference, y_mass_reference, best_jitterple, analysis_parameters, total_created_and_analyzed_samples_projection, output);
				//batchTestG(nl_test, pl_reference, nl_projected, *i_projections, ObliqueAspect, meridians, parallels, faces_test, iterations, alpha_mult, nu, eps, max_iter, max_diff, x_mass_reference, y_mass_reference, n_par, best_jitterple, analysis_parameters, sl, total_created_and_analyzed_samples_projection, output);
				...
				//if (total_created_and_analyzed_samples_projection_before < total_created_and_analyzed_samples_projection)
				{
					...
					//                 ( X ( 2, 0 ) >= MIN_LON ) && ( X ( 2, 0 ) <= lonp_interval_heur.max_val ) ) ) && ( ( X ( 3, 0 ) >= lat0_interval.min_val ) && ( X ( 3, 0 ) <= lat0_interval.max_val ) ) )
					{
						...
						{
							best_jitterple.setLatP(X(0, 0));
							best_jitterple.setLonP(X(1, 0));
							best_jitterple.setLat0(X(2, 0));
							best_jitterple.setLon0(X(3, 0));
						}
		...
		for (; (latp == MAX_LAT ? lonp == 0.0 : (lonp >= lonp_min) && (lonp <= lonp_max)); lonp += analysis_parameters.lonp_step)
		{
				...
				((latp >= latp_interval_heur.min_val) && (latp <= latp_interval_heur.max_val)))
			{
				...
				for (T lat0 = (lat0_set ? lat0_old : proj->getLat0Interval().min_val); (lat0 <= proj->getLat0Interval().max_val); lat0 += analysis_parameters.lat0_step)
				{
					...
					//Compute coordinates of all geographic points points in jitterple's projection + complex criterium
...
T CartAnalysis::computeAnalysisForOneSample(Container <Node3DCartesian <T> *> &nl_test, Container <Point3DGeographic <T> *> &pl_reference, typename TMeridiansList <T> ::Type & meridians, typename TParallelsList <T> ::Type & parallels,
	const Container <Face <T> *> &faces_test, Projection <T> *proj, const TAnalysisParameters <T> & analysis_parameters, Sample <T> &jitterple_res, bool singular_points_found, unsigned int & created_samples, std::ostream * output)
{
	//Compute all cartometric analysis for one jitterple
	...
	jitterple_res.setNonSingularPointsIndices(non_singular_points);

	//Create empty list of projected points and transformed points
	Container <Node3DCartesianProjected <T> *> nl_projected;

	//Get rotation
	const T alpha = jitterple_res.getAlpha();

	//Compute coordinates of all geographic points points in jitterple's projection and add to the list
	...
	//Set  k-best points
	jitterple_res.setKBestPointsIndices(k_best_points);
		...
		(!analysis_parameters.perform_heuristic))
	{
		...
		jitterple_res.setProj(proj);
		jitterple_res.setR(proj->getR());
		jitterple_res.setLatP(proj->getCartPole().getLat());
		jitterple_res.setLonP(proj->getCartPole().getLon());
		jitterple_res.setLat0(proj->getLat0());
		jitterple_res.setLon0(proj->getLon0());
		jitterple_res.setDx(proj->getDx());
		jitterple_res.setDy(proj->getDy());
		jitterple_res.setSingularPointsFound(singular_points_found);
		jitterple_res.setOutliersFound(outliers_found);
		jitterple_res.setNonSingularPointsIndices(non_singular_points);
		jitterple_res.setKBestPointsIndices(k_best_points);

		//Set pointer to a jitterple
		Sample <T> *p_jitterple = &jitterple_res;

		//2D Helmert transformation: jittere results for rotated and unrotated jitterple
		if (analysis_parameters.analysis_type.a_helt)
			analyzeSampleHelmertTransformationDeviation(*p_jitterple, *p_nl_test_best, *p_nl_projected_best, analysis_parameters.match_method, mult_ratio);

		//Create list for rotated test points and rotated jitterple as the copy of the result jitterple
		Container <Node3DCartesian <T> *> nl_test_best_rot;
		Sample <T> jitterple_rot(jitterple_res);

		//Process unrotated and rotated jitterple
		bool rotated_jitterple = false;

		//for (unsigned int j = 0; j < 2; j++)
		{
			//2D homothetic transformation:
			if (analysis_parameters.analysis_type.a_homt)
				analyzeSampleHomotheticTransformationDeviation(*p_jitterple, *p_nl_test_best, *p_nl_projected_best, analysis_parameters.match_method, mult_ratio);

			//Cross nearest distance
			if (analysis_parameters.analysis_type.a_cnd)
				analyzeSampleCrossNearestNeighbourDistance(*p_jitterple, *p_nl_test_best, *p_nl_projected_best, mult_ratio);

			//Analysis of graticule: turning function
			if (analysis_parameters.analysis_type.a_gn_tf)
				analyzeSampleGeographicNetworkTurningFunctionRatio(*p_jitterple, *p_nl_test_best, *p_nl_projected_best, *p_meridians_best, *p_parallels_best, mult_ratio);

			//Analysis of Voronoi diagrams: turning function
			if (analysis_parameters.analysis_type.a_vd_tf)
				analyzeSampleUsingVoronoiDiagramTurningFunctionRatio2(*p_jitterple, *p_nl_test_best, *p_nl_projected_best, faces_test, analysis_parameters, mult_ratio);

			//Angle of rotation given by 2D Helmert transformation
			const T rot_angle = p_jitterple->getRotation();

			//Is there a significant improvement for rotated jitterple: homt / helt > IMPROVE_RATIO_STD_DEV
			//Is the angle of rotation in  interval (88, 92) + k*M_PI/2
			rotated_jitterple = (analysis_parameters.correct_rotation) && (IMPROVE_RATIO_STD_DEV * jitterple_res.getHelmertTransformationRatio() < jitterple_res.getHomotheticTransformationRatio()) &&
				((short)(fabs(rot_angle) + REM_DIV_ROT_ANGLE) % 90 < 2 * REM_DIV_ROT_ANGLE) && (fabs(rot_angle) > MAX_LAT - REM_DIV_ROT_ANGLE);
			/*
			//Rotate a jitterple by the -( angle )
			if ((rotated_jitterple) && (!p_jitterple->getRotatedSample()))
			{
			...
			//Set pointer to rotated jitterple
			p_jitterple = &jitterple_rot;
			p_nl_test_best = &nl_test_best_rot;

			//Set angle and rotation flag for a jitterple
			p_jitterple->setRotatedSample(true);
			}
		...
		//Compute jitterple cost
		jitterple_cost = (rotated_jitterple ? jitterple_rot.getSampleCost(analysis_parameters.analysis_type) : jitterple_res.getSampleCost(analysis_parameters.analysis_type));

		//Increment successfully created samples for projection
		created_samples++;

		//Add also rotated jitterple to the list
		if (rotated_jitterple)
		{
			jitterple_res = jitterple_rot;
			created_samples++;
		}
	}

	//Return cost of the jitterple
	return jitterple_cost;
}
...
{
	//Small heuristic for jitterple: compare shape or the prime meridian, equator (central meridian, central parallel) north pole and south pole for test and reference points
	...
	//All test successfull, jitterple may be perspective
	return true;
}
...
{
	//Analyze jitterple using Homothetic transformation deviation
	try
	{
		...
		//Set ratio, percentage match, scale, tx and ty for the jitterple
...
{
	//Analyze jitterple using Helmert transformation deviation
	try
	{
		//List of transformed points
		Container <Node3DCartesian <T> *> nl_transformed;
		...
		//Set ratio and percentage match for the jitterple
...
{
	//Analyze jitterple using angular turning function differences in geographic network
...
{
//Analyze jitterple using Voronoi diagram ratios
try
{
...
//Error while jitterple processed or invalid configuration for cell by cell ratia
catch (Error & error)
{
...
{
	//Set position for each jitterple after its sorting
...
void CartAnalysis::batchTestShifts(Container <Node3DCartesian <T> *> &nl_test, Container <Point3DGeographic <T> *> &pl_reference, Container <Node3DCartesianProjected <T> *> &nl_projected, Projection <T> *proj, TProjectionAspect aspect, typename TMeridiansList <T> ::Type &meridians, typename TParallelsList <T> ::Type &parallels, const Container <Face <T> *> &faces_test,
	unsigned short  &iterations, const T alpha_mult, const T nu, const T eps, unsigned short max_iter, const T max_diff, T &x_mass_reference, T &y_mass_reference, Sample <T> &best_jitterple, TAnalysisParameters <T> & analysis_parameters, unsigned int &total_created_and_analyzed_samples_projection, std::ostream * output)
{
	...
	for (unsigned int i = 0; i < 11; i++)
	{
		...
		for (unsigned int j = 0; j< n_tests; j++)
		{
			...
			Sample <T> jitterple_test;
			//CartAnalysis::computeAnalysisForOneSample(nl_test_temp, pl_reference, meridians, parallels, faces_test, proj, analysis_parameters_test, jitterple_test, false, total_samples_test, output);

			//Get initialize Earth radius using the similarity transformation
			const T R_def = proj->getR() / jitterple_test.getScaleHelT();
			...
			//	analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W1, X1, Y1, V1, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
			...
			T min_cost = NonLinearLeastSquares::GND(FAnalyzeProjJ2 <T>(nl_test_temp, pl_reference, proj, aspect, analysis_parameters.print_exceptions, j1), FAnalyzeProjV2 <T>(nl_test_temp, pl_reference, meridians, parallels, faces_test, proj,
				analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W1, X1, Y1, V1, iterations, alpha_mult, eps, max_iter, max_diff, output);
			...
			//	analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
			...
			min_cost = NonLinearLeastSquares::GND(FAnalyzeProjJ <T>(nl_test_temp, pl_reference, proj, aspect, analysis_parameters.print_exceptions, j2), FAnalyzeProjV <T>(nl_test_temp, pl_reference, meridians, parallels, faces_test, proj,
				analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, iterations, alpha_mult, eps, max_iter, max_diff, output);
				...
				*/
				...
				if (test_gnd)
				{
					if (analysis_parameters.analysis_method == NonLinearLeastSquaresMethod)
						...
						analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, A, B, iterations, alpha_mult, eps, max_iter, max_diff, output);
					else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRotMethod)
						...
						x_mass_reference, y_mass_reference, analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, A, B, iterations, alpha_mult, eps, max_iter, max_diff, output);
					else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRot2Method)
						...
						R_def, q1, q2, analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, A, B, iterations, alpha_mult, eps, max_iter, max_diff, output);
					else if (analysis_parameters.analysis_method == NonLinearLeastSquaresShiftsMethod)
					...
					//	R_def, q1, q2, analysis_parameters, ObliqueAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X_init, Y, V, iterations, alpha_mult, nu, eps, 0.25 * max_iter, max_diff, output) : 0);
				...
				if (test_bfgsh)
				{
					...
					if (analysis_parameters.analysis_method == NonLinearLeastSquaresMethod)
						...
						analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, A, B, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
					else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRotMethod)
						...
						x_mass_reference, y_mass_reference, analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, A, B, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
					else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRot2Method)
						...
						R_def, q1, q2, analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, A, B, iterations, alpha_mult, nu, eps, max_iter, max_diff, output);
					else if (analysis_parameters.analysis_method == NonLinearLeastSquaresShiftsMethod)
				...
				if (test_lm)
				{
					...
					if (analysis_parameters.analysis_method == NonLinearLeastSquaresMethod)
						...
						analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, A, B, iterations, nu1, nu2, nu3, gamma1, gamma2, lambda_min, lambda_max, eps, max_iter, max_diff, delta, output);
					else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRotMethod)
						...
						x_mass_reference, y_mass_reference, analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, A, B, iterations, nu1, nu2, nu3, gamma1, gamma2, lambda_min, lambda_max, eps, max_iter, max_diff, delta, output);
					else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRot2Method)
						...
						R_def, q1, q2, analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, A, B, iterations, nu1, nu2, nu3, gamma1, gamma2, lambda_min, lambda_max, eps, max_iter, max_diff, delta, output);
					else if (analysis_parameters.analysis_method == NonLinearLeastSquaresShiftsMethod)
				...
				if (test_tr)
				{
					...
					R_def, q1, q2, analysis_parameters, ObliqueAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), WW, XXT2, YY, VV, iterations, nu1, nu2, nu3, gamma1, gamma2, lambda_min, lambda_max, eps, max_iter, max_diff, output) : 0);
					...
					//	R_def, q1, q2, analysis_parameters, ObliqueAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), WW, XXT2, YY, VV, iterations, alpha_mult, nu, eps, max_iter, max_diff, output) : 0);
					...
					*/
					...
					if (analysis_parameters.analysis_method == NonLinearLeastSquaresMethod)
						...
						analysis_parameters, ObliqueAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, A, B, iterations, nu1, nu2, nu3, gamma1, gamma2, lambda_min, lambda_max, eps, max_iter, max_diff, delta, output);
					else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRotMethod)
						...
						x_mass_reference, y_mass_reference, analysis_parameters, ObliqueAspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W2, X2, Y2, V2, A, B, iterations, nu1, nu2, nu3, gamma1, gamma2, lambda_min, lambda_max, eps, max_iter, max_diff, delta, output);
					else if (analysis_parameters.analysis_method == NonLinearLeastSquaresRot2Method)
...
template <typename T>
void CartAnalysis::batchTestSimplex(Container <Node3DCartesian <T> *> &nl_test, Container <Point3DGeographic <T> *> &pl_reference, Projection <T> *proj, TProjectionAspect aspect, typename TMeridiansList <T> ::Type &meridians, typename TParallelsList <T> ::Type &parallels, const Container <Face <T> *> &faces_test, const T R_init, const T latp_init, const T lonp_init, const T lat0_init, unsigned short &iterations, const T eps, unsigned short max_iter, const T max_diff, Sample <T> best_jitterple, TAnalysisParameters <T> & analysis_parameters, unsigned int &total_created_and_analyzed_samples_projection, std::ostream * output)
{
	//Perform batch tests
	unsigned short n_tests = 300;
	...
	for (int noise = 100; noise < 150; noise += 50)
	{
		...
		//for (T delta = 1; delta < 100000; delta *= 10)
		{
			...
			for (unsigned int j = 0; j < n_tests; j++)
			{
				...
				if (analysis_parameters.analysis_method == SimplexMethod)
					min_cost = SimplexMethod::NelderMead(FAnalyzeProjV2S <T>(nl_test, pl_reference, meridians, parallels, faces_test, proj, analysis_parameters, ObliqueAspect, best_jitterple,
					total_created_and_analyzed_samples_projection, it_res, output), XMIN, XMAX, W, X, Y, V, iterations, eps, max_iter, output);
				else if (analysis_parameters.analysis_method == SimplexRotMethod)
					min_cost = SimplexMethod::NelderMead(FAnalyzeProjV3S <T>(nl_test, pl_reference, meridians, parallels, faces_test, proj, analysis_parameters, ObliqueAspect, best_jitterple,
					total_created_and_analyzed_samples_projection, it_res, XMIN, XMAX, output), XMIN, XMAX, W, X, Y, V, iterations, eps, max_iter, output);
				else if (analysis_parameters.analysis_method == SimplexRot2Method)
					min_cost = SimplexMethod::NelderMead(FAnalyzeProjV4S <T>(nl_test, pl_reference, meridians, parallels, faces_test, proj, R_def, q1, q2, analysis_parameters, ObliqueAspect, best_jitterple,
					total_created_and_analyzed_samples_projection, it_res, output), XMIN, XMAX, W, X, Y, V, iterations, eps, max_iter, output);
				else if (analysis_parameters.analysis_method == SimplexShiftsMethod)
					min_cost = SimplexMethod::NelderMead(FAnalyzeProjVS <T>(nl_test, pl_reference, meridians, parallels, faces_test, proj, analysis_parameters, ObliqueAspect, best_jitterple,
					total_created_and_analyzed_samples_projection, it_res, output), XMIN, XMAX, W, X, Y, V, iterations, eps, max_iter, output);
	...
	for (int noise = 100; noise < 150; noise += 50)
	{
		...
		//for (T delta = 1; delta < 100000; delta *= 10)
		{
			...
			for (unsigned int j = 0; j < n_tests; j++)
			{
				...
				{
					if (analysis_parameters.analysis_method == DifferentialEvolutionMethod)
						min_cost = DifferentialEvolution::diffEvolution(FAnalyzeProjV2DE <T>(nl_test, pl_reference, meridians, parallels, faces_test, proj, analysis_parameters, ObliqueAspect, best_jitterple,
						...
						min_cost = DifferentialEvolution::diffEvolution(FAnalyzeProjV3DE <T>(nl_test, pl_reference, meridians, parallels, faces_test, proj, analysis_parameters, ObliqueAspect, best_jitterple,
						...
						min_cost = DifferentialEvolution::diffEvolution(FAnalyzeProjV4DE <T>(nl_test, pl_reference, meridians, parallels, faces_test, proj, R_def, q1, q2, analysis_parameters, ObliqueAspect, best_jitterple,
						...
						min_cost = DifferentialEvolution::diffEvolution(FAnalyzeProjVDE <T>(nl_test, pl_reference, meridians, parallels, faces_test, proj, analysis_parameters, ObliqueAspect, best_jitterple,
						total_created_and_analyzed_samples_projection, it_res1, output), XMIN, XMAX, population, eps, max_gen, F, CR, DERand1Strategy, Fixed, W, X, Y, V, XAVER, res_aver, res_max, iterations, output);
...
void CartAnalysis::batchTestG(Container <Node3DCartesian <T> *> &nl_test, Container <Point3DGeographic <T> *> &pl_reference, Container <Node3DCartesianProjected <T> *> &nl_projected, Projection <T> *proj, const TProjectionAspect aspect, typename TMeridiansList <T> ::Type &meridians, typename TParallelsList <T> ::Type &parallels, const Container <Face <T> *> &faces_test,
	unsigned short &iterations, const T alpha_mult, const T nu, const T eps, unsigned short max_iter, const T max_diff, T &x_mass_reference, T &y_mass_reference, const unsigned int n,  Sample <T> best_jitterple, TAnalysisParameters <T> & analysis_parameters, Container <Sample <T> > &sl, unsigned int &total_created_and_analyzed_samples_projection, std::ostream * output)
{
	...
	for (unsigned int i = 0; i < n_tests; i++)
	{
		...
		*/
		...
		min_cost = (analysis_parameters.analysis_method == NonLinearLeastSquaresMethod ?
			...
			analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X, Y, V, iterations, alpha_mult, nu, eps, max_iter, max_diff, output) :
			...
			x_mass_reference, y_mass_reference, analysis_parameters, aspect, best_jitterple, total_created_and_analyzed_samples_projection, output), FAnalyzeProjC <double>(), W, X, Y, V, iterations, alpha_mult, nu, eps, max_iter, max_diff, output));
		...
		//Correct result
		//if ( best_jitterple.getHomotheticTransformationRatio() < 5.0e6)
		//1.0e-4
		//3e0
		//5e6
		//3.0e-3
